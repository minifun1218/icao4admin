...
public class OpiController {
    ...
    public ResponseEntity<ApiResponse<Page<OpiQuestion>>> getQuestionsByTopicId(
        ...
        try {
            Sort sorting = Sort.by(Sort.Direction.fromString(sort[1]), sort[0]);
            Pageable pageable = PageRequest.of(page, size, sorting);
            Page<OpiQuestion> questions = opiQuestionService.findByTopicId(topicId, pageable);
    ...
    @PreAuthorize("hasRole('USER') or hasRole('ADMIN')")
    public ResponseEntity<ApiResponse<Map<String, Object>>> getTopicStats(@PathVariable Long topicId) {
        try {
            Map<String, Object> stats = new HashMap<>();
            
            // 基础统计
            stats.put("totalQuestions", opiQuestionService.countByTopicId(topicId));
            stats.put("questionsWithPromptText", 
                     opiQuestionService.findByTopicId(topicId).stream()
                             .mapToLong(q -> q.hasPromptText() ? 1 : 0).sum());
            stats.put("averageAnswerSeconds", opiQuestionService.getAverageAnswerSecondsByTopicId(topicId));
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<ApiResponse<List<OpiQuestion>>> exportQuestionsByTopic(@PathVariable Long topicId) {
        try {
            List<OpiQuestion> questions = opiQuestionService.exportQuestionsByTopic(topicId);
            return ResponseEntity.ok(ApiResponse.success(questions));
        } catch (Exception e) {
